  ...
  return (
    <div className="flex min-h-screen bg-slate-50">
      ...
      <div className="flex-1 flex flex-col overflow-auto pb-24 md:pb-0">
        ...
        {/* Title */}
        <div className="px-4 md:px-8 mt-5 mb-4 flex items-center gap-2">
        ...
        <div className="px-4 md:px-8 mb-4 flex items-center gap-3">
          ...
          <button className="flex items-center gap-1.5 px-3 py-2.5 border border-gray-200 rounded-xl text-sm text-gray-600 bg-white hover:bg-gray-50 transition shrink-0">
            <Filter size={14}/> <span className="hidden sm:inline">Filter</span>
          </button>
        </div>

        {/* Filter tabs */}
        <div className="px-4 md:px-8 mb-5 flex items-center gap-2 overflow-x-auto scrollbar-hide">
          {FILTER_TABS.map(tab => (
            <button key={tab} onClick={() => setActiveFilter(tab)}
              className={`px-3 py-1.5 rounded-full text-xs font-medium transition whitespace-nowrap ${activeFilter===tab ? "text-white" : "bg-white border border-gray-200 text-gray-600 hover:bg-gray-50"}`}
        ...
        {/* List */}
        <div className="flex-1 px-4 md:px-8 pb-8 space-y-3 overflow-auto">
          {filtered.length === 0 ? (
...
function TrashItem({ form, onRestore, onDelete }) {
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm flex items-start md:items-center gap-3 md:gap-4 p-3 md:p-4 hover:shadow-md transition">
      <div className="w-16 md:w-24 h-12 md:h-16 rounded-xl shrink-0 flex items-center justify-center" style={{background:form.gradient}}>
        <svg className="w-6 md:w-8 h-6 md:h-8 opacity-25" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-2 mt-1 text-xs text-gray-400 flex-wrap">
          ...
          <span className="hidden sm:inline">·</span><span className="hidden sm:inline">{form.responses.toLocaleString()} responses</span>
        </div>

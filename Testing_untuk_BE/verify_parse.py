"""Simulasi logika parsing TypeScript di Python untuk verifikasi"""
import zipfile, re
...
def get_text(xml):
    ...
    return decode_entities(''.join(parts).strip())
...
for p in paras:
    ...
    # ── Teks kosong setelah strip → hanya update metadata ─────────────────
    if not text:
        if current:
    ...
    if is_explicit_o or is_manual_o:
        ...
        if val: current['options'].append({'value': val})
        continue
...
for i, q in enumerate(questions):
    q_preview = q["question"][:90].replace('\n', ' | ')
    print(f'Soal {i+1:2d}: [{q["type"]:8s}] {q_preview}')
    if '```' in q['question']:
        print(f'           ✅ KODE BLOCK TERDETEKSI')

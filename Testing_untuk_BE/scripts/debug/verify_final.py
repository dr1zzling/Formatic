"""Verifikasi parser dengan soal-template.docx (punya Cambria Math)"""
import zipfile, re

PATH = '/home/erzy/projekKelompok/FormMaker/Testing_untuk_BE/soal-template.docx'
...
def is_math_para(xml):
    return bool(re.search(r'w:ascii="(?:Cambria Math|Latin Modern Math|XITS Math|Asana Math|Neo Euler|TeX Gyre)"', xml, re.I))
...
# Show paragraphs with font info for math check
print("\n=== Paragraf 24-39 (soal 6-7) ===")
for i in range(24, 40):
    if i >= len(paras): break
    p = paras[i]
    t = decode(''.join(re.findall(r'<w:t[^>]*>([^<]*)</w:t>', p)).strip())
    is_c = is_code_para(p)
    is_m = is_math_para(p)
    if t or is_c or is_m:
        flag = '[MATH]' if is_m else ('[CODE]' if is_c else '')
        print(f"[{i:03d}]{flag} {repr(t[:70])}")
...
def flush_code():
    global code_buffer
    while code_buffer and code_buffer[-1] == '': code_buffer.pop()
...
def flush_math():
    global math_buffer
    while math_buffer and math_buffer[-1] == '': math_buffer.pop()
    if not math_buffer: return None
    block = '\n'.join(f'$${line}$$' for line in math_buffer)
    math_buffer = []
    return block
...
for p in paras:
    ...
    if not text and not has_img and math_buffer:
        math_buffer.append('')
        continue

    if code_buffer:
        block = flush_code()
        if block and current: current['question'] += '\n' + block
    ...
    if is_explicit_q or (is_manual_q and (not current or current['options'] or current.get('type') is not None)):
        finish_q()
        current = {'question': re.sub(r'^\d+[.\)]\s*', '', text),
...
print(f'\n=== Total soal ter-parse: {len(questions)} ===\n')
for i, q in enumerate(questions):
    q_prev = q['question'][:110].replace('\n', ' ↵ ')
    print(f'Soal {i+1:2d}: [{q["type"] or "?":8s}] {q_prev}')
    if '```' in q['question']: print('           ✅ CODE BLOCK')
    if '$$'  in q['question']: print('           📐 MATH BLOCK')
    for j, o in enumerate(q['options']):
        c = '✓' if o.get('is_correct') else ' '
        print(f'   [{c}] {chr(65+j)}: {o["value"][:65]}')
    print()
